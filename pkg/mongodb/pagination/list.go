...
type Paginator interface {
	...
	RunQuery(ctx context.Context, fromCollection *mongo.Collection,
		fromCollectionColumnsMap map[string]Columned, defaultSort string, dataSliceType interface{})
}
...
type pipeline struct {
	Pipeline              bson.D
	ChangeableResultCount bool // ChangeableResultCount this field is true when stage is match, group or
	// another stages that can change result count
	ResultColumns map[string]Columned
}

// New create pagination list
func New() Paginator {
	return &List{
...
// AddPipelineStages add multiple stages to pipeline and ChangeableResultCount
// this field is true when stage is match, group or another filter stages
...
// AddPipelineStage add multiple stages to pipeline and ChangeableResultCount
// this field is true when stage is match, group or another filter stages
...
func (list *List) RunQuery(ctx context.Context, fromCollection *mongo.Collection,
	fromCollectionColumnsMap map[string]Columned, defaultSort string, dataSliceType any,
) {
	...
	list.setListTotalCount(ctx, countPipelines)
	...
	list.prepareResult(ctx, cursor, dataSliceType)
}

func (list *List) generateFilterStage(filters map[string]string,
	columnsMap map[string]Columned,
) (stage bson.D, remainingFilters map[string]string) {
	filterStage := bson.D{}
	for filterCol, filterValue := range filters {
		if columnInfo, ok := columnsMap[filterCol]; ok {
			filterStage = append(filterStage, bson.E{
				Key:   columnInfo.(*ColumnInfo).Column,
				Value: columnInfo.getFilterVal(filterValue),
			})
...
func (list *List) mergePaginationPipelines() (dataPipeline mongo.Pipeline, countPipeline mongo.Pipeline) {
	...
	for i := pipelineIndex; i < len(list.dbPipelines); i++ {
		pipe := list.dbPipelines[i]
		preparedPipelines = append(preparedPipelines, pipe.Pipeline)
		if pipe.ResultColumns != nil {
			preparedPipelines = list.addSortAndPageLimitStages(preparedPipelines, pipe.ResultColumns)
		}
	}
...
func (list *List) addSortAndPageLimitStages(preparedPipelines mongo.Pipeline,
	columnsMap map[string]Columned,
) mongo.Pipeline {
	if !list.sortStageAdded {
		if list.Sort == "" {
...
func (list *List) setListTotalCount(context context.Context, countPipeline mongo.Pipeline) {
	...
	countData := bson.M{}
	_ = countCursor.Decode(&countData)
...
func (list *List) prepareResult(context context.Context, cursor *mongo.Cursor, dataSliceType interface{}) {
	list.prepareJSONResult(context, cursor, dataSliceType)
}

func (list *List) prepareJSONResult(context context.Context, cursor *mongo.Cursor, dataSliceType interface{}) {
...
func (list *List) addChangeableResultCountStagesAndFilters(
	remainingFilters map[string]string,
) ([]bson.D, int, map[string]string) {
	pipelineIndex := 0
	var pipelines []bson.D
	filterStage := bson.D{} //nolint
	/// add pipelines before sort and pagination stages that maybe change result count
